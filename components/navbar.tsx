  ...
  return (
    <nav className="flex justify-between items-center max-w-6xl mx-auto py-8 px-12 text-zinc-600 text-sm font-semibold dark:text-white dark:border-b-zinc-800 border-zinc-200 border-b ">
      ...
      <div className="flex justify-center gap-6 ">
        <Link
          href={"/about"}
          className={`dark:hover:text-green-400 hover:text-black ${curpage === '/about' && 'text-green-500'}`}
        >
          About
        </Link>
        <Link
          href={"/projects"}
          className={`dark:hover:text-green-400 hover:text-black ${curpage === '/projects' && 'text-green-500'}`}
        >
          Projects
        </Link>
        {/* <Link
          href={"/blog"}
          className={`dark:hover:text-green-400 hover:text-black ${curpage === 'blog' && 'text-green-500'}`}
        >
          Blog
        </Link> */}
      </div>
